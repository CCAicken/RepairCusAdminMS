...
<form id="div_addcar"
	...
	<form id="div_edlcar" lay-filter="edtdate"
		...
		<div class="layui-form-item">
            <label class="layui-form-label">车辆品牌:</label>
            <div class="layui-input-inline">
            <input type="text" name="brand"   lay-verify="addtitle"
		...
		<div class="layui-form-item">
		 ...
		 <input id="edtcarmileage"   name="carmileage"   value="true" class="layui-input"   style="display:none" /> 
		 <input id="edtstate"   name="state"   value="true" class="layui-input"   style="display:none" /> 
		 <input id="edtcustomeraccount"   name="customeraccount"    class="layui-input"   style="display:none" />
		 <input id="edtcustomercarid"   name="customercarid"    class="layui-input"   style="display:none" />
		 <input id="edtchassisnumber"   name="chassisnumber"    class="layui-input"   style="display:none" />
		 <input id="edtenginenumber"   name="enginenumber"    class="layui-input"   style="display:none" />
		 <input id="edtlicenseplatenumber"   name="licenseplatenumber"    class="layui-input"   style="display:none" />
	</form>
  	...
  	layui.use(['layer','upload','table','form','enhanceform'], function(){
		...
		$("#btnselfrontinfo").click(function() {
			table.reload('classlist', {
				method : 'post',
				where : {
					'realname' : $("#input_wherecondition").val().trim(),
					'cartypeid':$("#collegeselect").val()
						},
				page : {
					curr : 1
					}
				...
				case 'del':
					layer.confirm('即将删除'+data.carname+',确定要删除么？', {
					  ...
					  icon:3
						...
						$.ajax({
			        		type: 'get',
			        		url: "../customercar/delcustomercar",
			        		dataType: 'json',
			        		data:{
			        		classid:data.carid
			        		},
 						...
 						success: function (layero) { 
  						...
  						yes: function (layero, index) { 
		                     ...
		                     if($("#edtphotoid").val()==""){ 
		   					...
		   					$.ajax({
				        		type: 'post',
				        		url: "../customercar/updatecustomercar",
				        		dataType: 'json',
				        		data: formData ,
				        		success:function(data){
				        			if(data.code == 0){
  				...
  				content: $('#div_addcar'),
  				 yes: function (layero, index) { 
  				   form.on('submit(formDemo)', function (data) {
   					...
   					$.ajax({
		        		type: 'post',
		        		url: "../customercar/addcustomercar",
		        		dataType: 'json',
		        		data: formData ,
		        		success:function(data){
		        			if(data.code == 0){
